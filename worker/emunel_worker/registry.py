"""Persistent instance registry for the EMUNEL Worker.
...
def _path() -> Path:
    base = Path(os.environ.get("EMUNEL_WORKER_DATA", "/data/instances"))
...
def save_instance(instance_id: str, data: dict) -> None:
    ...
    try:
        ...
        get("runtime", "emunel.worker.registry").warning("registry save failed: %s", exc)
