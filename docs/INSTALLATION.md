...
1. **Clone and create databases**
   ...
   ```bash
   git clone <your-fork> emunel && cd emunel
   createdb emunel
   ```
...
3. **GitHub OAuth App** — callback `http://127.0.0.1:8080/auth/callback`
   (or your public URL). Set `EMUNEL_GITHUB_CLIENT_ID` / `..._SECRET`.
...
4. **Run** — see `deploy/scripts/dev.sh`, or set the environment variables
   ...
   ```bash
   # console
   (cd console/api && .venv/bin/python -m emunel_console)         # :8080
   # worker
   EMUNEL_CORE_PYTHON=$PWD/core/.venv/bin/python \
   EMUNEL_CORE_CWD=$PWD/core \
   (cd worker && .venv/bin/python -m emunel_worker)               # :9100
   ```
