...
Console  ──(EMUNEL_WORKER_TOKEN)───────────▶ Worker
Worker   ──(per-instance EMUNEL_CORE_API_TOKEN)──▶ EMUNEL Core
...
- Cookies: `HttpOnly`, `SameSite=Lax`, `Secure` in production
  (`EMUNEL_COOKIE_SECURE=1`), 7-day TTL, max 20 sessions per user.
- **CSRF**: every mutating request must repeat the session token in
  `X-EMUNEL-CSRF` (synchronizer-token; the cookie is unreadable to other
...
- **X-Forwarded-For trust**: client IP logging trusts proxy headers; on
  managed platforms the edge sets them. Direct unproxied
...
- **Worker token is shared**: suitable for one operator's nodes; multi-tenant
  ...
  interface; see `TODO(per-node-tokens)` in `worker/emunel_worker/main.py`).
