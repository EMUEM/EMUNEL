...
def check_csrf(request: Request, user: asyncpg.Record | None) -> None:
    ...
    the X-EMUNEL-CSRF header. This works because the cookie is HttpOnly and
    ...
    header_token = request.headers.get("x-emunel-csrf") or ""
    if not header_token or not secrets.compare_digest(header_token, cookie_token):
        raise HTTPException(status_code=403, detail="CSRF token missing or invalid")


def csrf_token(request: Request) -> str:
    """Value the frontend must send in X-EMUNEL-CSRF (equal to the cookie)."""
