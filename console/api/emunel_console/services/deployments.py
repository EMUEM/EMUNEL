...
Providers:
* ``railway`` — one Railway service per EMUNEL instance (per-instance public
  domain via Railway's API, WebSocket-capable, TLS at the edge). Selected
  automatically when EMUNEL_RAILWAY_TOKEN + project/environment are configured.
* ``local``   — EMUNEL Worker on a node running instances as containers
...
log = get("runtime", "emunel.console.deploy")
...
async def _pipeline_railway(pool: asyncpg.Pool, deployment_id: str, inst: dict,
    ...
    if not service_id:
        ...
        service_id = await provider.create_service(f"emunel-inst-{instance_id[:8]}")
        await pool.execute(
...
async def _pipeline_local(pool: asyncpg.Pool, deployment_id: str, inst: dict) -> None:
    ...
    await _set_deployment(pool, deployment_id, "building")
    await _log(pool, deployment_id, f"Preparing EMUNEL Core {inst['core_version']}…")
