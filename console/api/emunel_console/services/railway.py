...
EMUNEL's "cloud" provider for Railway (https://railway.com). Railway's
...
* Additional public endpoints are port-forwards (TCP proxies) — not needed by
  EMUNEL Core v1 since every supported protocol (VLESS / Trojan / Shadowsocks
...
One EMUNEL instance = one Railway service in the EMUNEL project, deployed from
a container image (default ``ghcr.io/emunelsh/emunel-core:latest``), with a
...
log = get("runtime", "emunel.console.railway")

GRAPHQL_URL = "https://backboard.railway.app/graphql/v2"

# Railway deployment status -> EMUNEL deployment status
STATUS_MAP = {
    "QUEUED": "queued",
...
class RailwayProvider:
    def __init__(self, token: str, project_id: str, environment_id: str,
                 core_image: str = "ghcr.io/emunelsh/emunel-core:latest"):
        self.token = token
    ...
    def from_env(cls) -> "RailwayProvider":
        token = os.environ.get("EMUNEL_RAILWAY_TOKEN", "")
        if not token:
            raise RailwayError("EMUNEL_RAILWAY_TOKEN is not set")
        project = os.environ.get("EMUNEL_RAILWAY_PROJECT_ID") or os.environ.get("RAILWAY_PROJECT_ID", "")
        environment = (
            os.environ.get("EMUNEL_RAILWAY_ENVIRONMENT_ID") or os.environ.get("RAILWAY_ENVIRONMENT_ID", "")
        )
        if not project or not environment:
            raise RailwayError(
                "EMUNEL_RAILWAY_PROJECT_ID / EMUNEL_RAILWAY_ENVIRONMENT_ID must be set "
                "(they are auto-injected when the Console itself runs on Railway)"
            )
        image = os.environ.get("EMUNEL_CORE_IMAGE", "ghcr.io/emunelsh/emunel-core:latest")
        return cls(token, project, environment, image)

    @staticmethod
    def available() -> bool:
        return bool(
            os.environ.get("EMUNEL_RAILWAY_TOKEN")
            and (
                os.environ.get("EMUNEL_RAILWAY_PROJECT_ID")
                or os.environ.get("RAILWAY_PROJECT_ID")
            )
            and (
                os.environ.get("EMUNEL_RAILWAY_ENVIRONMENT_ID")
    ...
    async def _gql(self, query: str, variables: dict[str, Any] | None = None,
        ...
        if resp.status_code == 401:
            raise RailwayError("Railway token rejected (401) — check EMUNEL_RAILWAY_TOKEN")
    ...
    async def configure_service(self, service_id: str, core_api_token: str, public_host: str) -> None:
        ...
        variables = {
            "EMUNEL_CORE_API_TOKEN": core_api_token,
            "EMUNEL_STATE_PATH": "/data/state.json",
            "PORT": "8000",
        }
        if public_host:
            variables["EMUNEL_PUBLIC_HOST"] = public_host
