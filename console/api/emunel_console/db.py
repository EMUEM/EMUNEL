"""Database layer for the EMUNEL Console.
...
log = get("runtime", "emunel.console.db")
...
async def _connect_with_retry(dsn: str, ssl_ctx: ssl_module.SSLContext | None) -> asyncpg.Pool:
    ...
    for attempt in range(1, 31):
        try:
            ...
            raise RuntimeError(
                "PostgreSQL rejected the credentials in EMUNEL_DATABASE_URL / DATABASE_URL. "
...
async def init_pool() -> None:
    ...
    if not dsn:
        ...
        base = None
        for cand in (Path("/data"), Path.cwd() / ".emunel-data"):
            try:
                ...
                continue
        base = base or Path("/tmp/emunel-data")
        base.mkdir(parents=True, exist_ok=True)
        sqlite_dsn = f"sqlite:///{base / 'emunel.db'}"
    ...
    await _seed_default_admin()
    log.info("EMUNEL Console database: PostgreSQL at %s", _mask_dsn(normalized))
