...
DSN = os.environ.get("EMUNEL_TEST_DSN", "postgresql://admin:emunel@127.0.0.1:5432/emunel")
CONSOLE = os.environ.get("EMUNEL_TEST_CONSOLE", "http://127.0.0.1:8080")
WORKER = os.environ.get("EMUNEL_TEST_WORKER", "http://127.0.0.1:9100")
WORKER_TOKEN = os.environ.get("EMUNEL_WORKER_TOKEN", "dev-worker-token-0123456789abcdef")
...
async def test_deploy_and_relay():
    ...
    os.environ.setdefault("EMUNEL_DATABASE_URL", DSN)
    os.environ.setdefault("EMUNEL_WORKER_TOKEN", WORKER_TOKEN)
    os.environ.setdefault("EMUNEL_LOCAL_WORKER_URL", WORKER)
    from emunel_console.services import deployments
    ...
    assert b"HELLO-EMUNEL" in resp[2:], f"payload missing: {bytes(resp[2:120])!r}"
    srv.shutdown()
    await pool.close()
